...
export interface AmountStepProps {
  ...
  error: string | null;
  onBack: () => void;
  onNext: (amountSats: number, sendAll?: boolean) => void;
}
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <input
          type="number"
          value={localAmount}
          onChange={(e) => { setLocalAmount(e.target.value); setIsSendAll(false); }}
        ...
        <div className="flex gap-2 mt-3">
          {[100, 1000, 10000, 100000].map((quickAmount) => (
            <button
              key={quickAmount}
              onClick={() => { setLocalAmount(String(quickAmount)); setIsSendAll(false); }}
        ...
        {balanceSats !== undefined && balanceSats > 0 && (
          <div
            className="flex items-center justify-between mt-3 p-3 rounded-xl border border-spark-border cursor-pointer select-none"
            onClick={() => {
              const next = !isSendAll;
              setIsSendAll(next);
              if (next) setLocalAmount(String(balanceSats));
            }}
            data-testid="send-all-toggle"
          >
            <div>
              <span className="text-sm font-medium text-spark-text-primary">Use All Funds</span>
              <p className="text-xs text-spark-text-secondary mt-0.5">
                {balanceSats.toLocaleString('en-US').replace(/,/g, '\u2009')} sats available
              </p>
            </div>
            <button
              type="button"
              role="switch"
              aria-checked={isSendAll}
              className={`relative inline-flex h-6 w-11 shrink-0 rounded-full transition-colors ${
                isSendAll ? 'bg-spark-warning' : 'bg-spark-border'
              }`}
              data-testid="send-all-switch"
            >
              <span
                className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform transition-transform mt-0.5 ${
                  isSendAll ? 'translate-x-[22px]' : 'translate-x-0.5'
                }`}
              />
            </button>
          </div>
      ...
      <div className="flex gap-3">
        ...
        <PrimaryButton
          onClick={() => validAmount && onNext(parseInt(localAmount), isSendAll)}
