...
export const getAllLogsAsZip = async (): Promise<Blob> => {
  ...
  const currentSessionHeader = [
    brand.logExportTitle,
    `Session: Current`,
    `Generated: ${now.toISOString()}`,
    '='.repeat(60),
    '',
  ].join('\n');
  zip.file(`${nowTimestamp}_${brand.logPrefix}_current.txt`, currentSessionHeader + '\n' + getAllLogs());
  ...
  if (isStorageAvailable()) {
    try {
      ...
      for (const session of sessions) {
        ...
        const filename = `${sessionTimestamp}_${brand.logPrefix}_session.txt`;
        const sessionHeader = [
          brand.logExportTitle,
...
export const shareOrDownloadLogs = async (): Promise<void> => {
  ...
  const filename = `${timestamp}_${brand.logPrefix}_logs.zip`;

  if (canShareFiles()) {
    const file = new File([blob], filename, { type: 'application/zip' });
    if (navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({ files: [file], title: brand.logShareTitle });
        return;
      } catch (e) {
